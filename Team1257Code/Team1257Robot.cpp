	...
	// sThingy(6)
{
	...
	sf = 0.7; // Change this to decrease drive speed
	//speedValue = 0;
...
void CTeam1257Robot::Autonomous()
{
	...
	move.Start();
	//aimRobot(camera);
	while(directionToDrive != 0)
	{
		move.Reset();
		if(directionToDrive == 1)
		{
			while(move.Get() < 1)
			{
				drive(0.4, 0.4);
...
void CTeam1257Robot::OperatorControl()
{
	...
	while(IsOperatorControl() && IsEnabled())
	{
		...
		{
			drive(-leftStick.GetY(),-leftStick.GetTwist());
...
{
	team1257Robot.SetLeftRightMotorOutputs(-leftStick.GetY() * sf, -leftStick.GetRawAxis(5) * sf);
